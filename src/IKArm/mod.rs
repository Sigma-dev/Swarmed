    ...
    for (arm_entity, arm) in arm_query.iter_mut() {
        for child in children_query.iter_descendants(arm_entity) {
            ...
            // println!("len: {}", rotation_arc.length());
                ...
                transform_params.p1().get_mut(skinned_mesh.joints[0]).unwrap().rotate_axis( Dir3::from_xyz(dir.x, dir.y, dir.z).unwrap(), signed_angle); // we already verified this exists above
            };
