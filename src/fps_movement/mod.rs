...
impl Default for MovementBundle {
    fn default() -> Self {
        Self::new(500.0, 0.9, 7.0, PI * 0.45)
    }
}
            ...
            &mut controllers
        {
            ...
            match event {
                MovementAction::Move(direction) => {
                    //let move_vec: Vec3 = transform.right() * direction * movement_acceleration.0 * delta_time;
                    let mut move_vec: Vec3 = transform.right() * direction.x * movement_acceleration.0 * delta_time;
                    move_vec += transform.forward() * direction.y * movement_acceleration.0 * delta_time;
                    move_vec = move_vec.normalize();

                    linear_velocity.x += move_vec.x;
                    linear_velocity.z += move_vec.z;
                }
                MovementAction::Jump => {
                    if is_grounded {
